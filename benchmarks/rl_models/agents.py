...
from data_structures import Dict
...
class AgentBase(ABC):
    ...
    @abstractmethod
    def greedy_policy(self, max_steps, **kwargs):
        """
...
class CrossEntropyAgent(AgentBase):
    ...
    def act(self, obs: np.ndarray[Tuple[int]]):
        ...
        reveal_type(e_Z_temp)
        action_probs = e_Z / e_Z_temp
        reveal_type(action_probs)
    ...
    def run_episode(self, max_steps, render=False):
        ...
        x = np.mean(D["cumulative_rewards"])
        y = np.mean(steps)
        reveal_type(x)
        reveal_type(y)
        return x , y

    def _episode(self, W : np.ndarray[Tuple[int, int]], b : np.ndarray[Tuple[int]], max_steps : int, render):
        """
    ...
    def update(self):
        ...
        # TODO here the input to the slice is unknown, so the output is too, not sure how to handle that
        sorted_y_val_idxs = np.argsort(D["cumulative_rewards"])[::-1]
        reveal_type(sorted_y_val_idxs)
    ...
    def greedy_policy(self, max_steps, render=True, **kwargs):
        """
        ...
        # TODO Changed reshape to take a tuple input instead
        temp = sample[0, :W_len]
        reveal_type(temp)
        W = temp.reshape((obs_dim, n_actions))
        reveal_type(W)
        b = sample[0, W_len:]
        reveal_type(b)
...
class MonteCarloAgent(AgentBase):
    ...
    def _init_params(self):
        ...
        n_states : int = np.prod(E["n_obs_per_dim"])
        n_actions : int = np.prod(E["n_actions_per_dim"])
        ...
        if self.off_policy:
            test = np.zeros((n_states, n_actions))
            self.parameters["C"] = test
            reveal_type(test)
    ...
    def _epsilon_soft_policy(self, s, a=None):
        ...
        p_a : int = self.epsilon / n_actions

        temp = np.ones(n_actions)
        reveal_type(temp)
        reveal_type(p_a)
        action_probs =  temp * p_a
        reveal_type(action_probs)
