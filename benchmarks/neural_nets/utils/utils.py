from typing import Tuple, Generator, Union, Optional, Any
...
def minibatch(X: np.ndarray[Tuple[int], np.dtype[Any]], batchsize: int = 256, shuffle: bool = True):
    """
    ...
    def mb_generator():
        for i in range(n_batches):
            yield ix[i * batchsize : (i + 1) * batchsize] # ignore
...
def calc_pad_dims_1D(
    ...
    padding_array = np.array([pw1, pw2])
    reveal_type(padding_array)
    if any(padding_array < 0):
        raise ValueError("Padding cannot be less than 0. Got: {}".format((pw1, pw2)))
    return (pw1, pw2)


def pad1D(
    X: np.ndarray[Tuple[int, int, int], np.dtype[Any]],
    pad,
...
def pad2D(
    X: np.ndarray[Tuple[int, int, int, int], np.dtype[Any]],
    pad,
...
def dilate(X: np.ndarray[Tuple[int, int, int, int], np.dtype[Any]], d: int):
#     """
...
def calc_fan(weight_shape: Tuple[int, ...]) -> Tuple[int, int]:
#     """
...
# Assuming 3D convolution for type annotation
def calc_conv_out_dims(
    X_shape: Tuple[int, int, int],
...
#     # k.shape = (fr * fc * n_in, 1)
    ...
    # i1/j1 : row/col templates
    # i0/j0 : n. copies (len) and offsets (values) for row/col templates
    arange_fr = np.arange(fr)
    reveal_type(arange_fr)
    i0 = np.repeat(arange_fr, fc)
    reveal_type(i0)

    offset = d + 1
    i0 = np.tile(i0, n_in) * offset
    reveal_type(i0)

    arange_out_rows = np.arange(out_rows)
    reveal_type(arange_out_rows)
    repeated_i1 = np.repeat(arange_out_rows, out_cols)
    reveal_type(repeated_i1)
    i1 = s * repeated_i1
    reveal_type(i1)

    arange_fc = np.arange(fc)
    reveal_type(arange_fc)
    j0 = np.tile(arange_fc, fr * n_in) * offset
    reveal_type(j0)

    arange_out_cols = np.arange(out_cols)
    reveal_type(arange_out_cols)
    tiled_j1 = np.tile(arange_out_cols, out_rows)
    reveal_type(tiled_j1)
    j1 = s * tiled_j1
    reveal_type(j1)
    ...
    arange_n_in = np.arange(n_in)
    reveal_type(arange_n_in)
    repeated_k = np.repeat(arange_n_in, fr * fc)
    reveal_type(repeated_k)
    k = repeated_k.reshape(-1, 1)
    reveal_type(k)
    reveal_type(i)
    reveal_type(j)
    return k, i, j
